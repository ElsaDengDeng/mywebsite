...
EventHandlerUtil.addHandler(elementControl, 'click', function (e) {
	...
	if (target.tagName === 'LI') {
		...
		if (targetValue === 'c') {
			...
			//普通四则运算符号
			//考虑到连续运算的可能，每次点过计算符号后先运算之前的操作符，再获取当前的运算符
			calculate();
			op = targetValue;
		} else {
			// 只按下=的情况
...
var calculate = function () {
	if (n2 === null) {
		// 如果现在n2为null，说明此时没有输入第二个数字，需要做的操作是交换变量，清空暂存数据
		n2 = n1;
		reset();
	} else {
		//这种情况下可以肯定第一组数据完备，n1 n2都有值，如果还没被按下运算符号说明用户是要连续计算了
		switch (op) {
			case '+':
				n2 += n1;
				reset();
